...
export default function SpreadsheetEditHistory({
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-tera-primary">Edit History</h3>
        <div className="text-tera-secondary text-sm">Loading...</div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-tera-primary">Edit History</h3>
        <div className="text-sm tera-status-error rounded-lg px-3 py-2">Error: {error}</div>
        <button
          onClick={fetchEditHistory}
          className="text-sm tera-link-accent"
        >
          Try again
        </button>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {edits.length === 0 ? (
        <div className="text-tera-secondary text-sm">No edits yet</div>
      ) : (
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {edits.map((edit) => (
            <div
              key={edit.id}
              className="rounded-lg tera-surface-subtle p-3 space-y-1"
            >
              <div className="flex items-center justify-between">
      ...
      <button
        onClick={fetchEditHistory}
        className="text-xs tera-link-accent"
      >
        Refresh
      </button>
    </div>
